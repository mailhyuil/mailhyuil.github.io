...
원시값을 다룰 때
객체를 통째로 교체해야할 때
컴포넌트 사이 값을 주고받을 때 반응성을 안전하게 유지하고 싶을때
...
값이 완전히 교체되는 경우 reactive()는 반응성을 잃는다. ref()는 반응성을 유지한다.
...
```vue
<Inquiry :is-open="isInquiryOpen" @dismiss="dismiss" @click="isInquiryOpen = true">
</Inquiry>
```
