# SSL
> 보안 소켓 계층 (Secure Sockets Layer)을 이르는 말로, 인터넷에서 데이터를 안전하게 전송하기 위한 인터넷 통신 규약 *프로토콜*이다
>> 암호화 기반 인터넷 보안 프로토콜로서, 개인정보 보호, 인증, 무결성을 인터넷 통신에 제공한다고 한다. SSL은 현재 TLS(Transport Layer Security)로 발전했다
>>> https : 웹에서 전송되는 데이터를 암호화여 데이터를 가로채더라도 거의 해독할 수 없는 복잡한 문자를 보게 된다.
