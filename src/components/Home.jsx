...
const Home = () => {
    return (
        <motion.div className="h-full bg-hyuil bg-cover flex justify-center items-center font-primary rounded"
            initial={{ x: -100, y: -100 }}
            animate={{ color: 'rgb(250,200,0)', x: 0, y: 0 }}
            transition={{ delay: .2, type: 'spring', stiffness: 500 }}>
            <motion.div className="bg-white/40 p-10 rounded-xl flex flex-col items-center shadow-xl"
                initial={{ x: -100, y: -100 }}
                animate={{ color: 'rgb(250,200,0)', x: 0, y: 0 }}
                transition={{ delay: .2, type: 'spring', stiffness: 500 }}>
